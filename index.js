...
const users = [{ id: '1', name: 'Raghuveer K', password: 'pwd', email: 'rk@gmail.com' }];
...
app.get('/vans/:id', (req, res) => {
    let van = vans.find(van => van.id === req.params.id);
    if (!van) van = { id: 'none', name: 'Not found', description: 'does not exit', type: '', imageUrl: '', price: 0 }
    res.json(van);
});
...
app.listen(port, () => {
    console.log(`Server started successfulyy on, ${port}`);
})